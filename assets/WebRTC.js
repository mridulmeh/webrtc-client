...
function initSymple()
{
	let gameURL = window.location.href.substr(0, window.location.href.indexOf(":",10))+':'+'4501';
	let clientConfig = {
		url: gameURL,		
	};
	clientConfig.peer={
	...
	currentPeerConnection.createAnswer(	
		function(desc) { // success
			...
			if(desc.sdp.indexOf("b=AS:30") < 0)	{
				...
				let First = desc.sdp.slice(0,StartIndex);
				let Last = desc.sdp.slice(StartIndex);
